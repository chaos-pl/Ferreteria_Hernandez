            ...
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <select name="categorias_id" class="form-select @error('categorias_id') is-invalid @enderror" required>
                        ...
                        @foreach($categorias as $c)
                            <option value="{{ $c->id }}" {{ old('categorias_id')==$c->id?'selected':'' }}>{{ $c->nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <select name="unidades_id" class="form-select @error('unidades_id') is-invalid @enderror" required>
                        ...
                        @foreach($unidades as $u)
                            <option value="{{ $u->id }}" {{ old('unidades_id')==$u->id?'selected':'' }}>{{ $u->nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <select name="marcas_id" class="form-select @error('marcas_id') is-invalid @enderror">
                        ...
                        @foreach($marcas as $m)
                            <option value="{{ $m->id }}" {{ old('marcas_id')==$m->id?'selected':'' }}>{{ $m->nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" name="existencias" min="0" class="form-control @error('existencias') is-invalid @enderror"
                           value="{{ old('existencias',0) }}" required>
