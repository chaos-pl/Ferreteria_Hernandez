    ...
    <div class="container py-4">
        ...
        <form method="post" action="{{ route('admin.compras.update',$compra) }}">
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-4">
                    ...
                    <select name="proveedores_id" class="form-select" required>
                        @foreach($proveedores as $p)
                            <option value="{{ $p->id }}" @selected(old('proveedores_id',$compra->proveedores_id)==$p->id)>
                ...
                <div class="col-md-4">
                    ...
                    <input type="datetime-local" name="fecha_compra" class="form-control"
                           value="{{ old('fecha_compra', optional($compra->fecha_compra)->format('Y-m-d\TH:i')) }}" required>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select name="estado" class="form-select">
                        @foreach(['borrador','ordenada','recibida','cancelada'] as $e)
                            <option value="{{ $e }}" @selected(old('estado',$compra->estado)==$e)>{{ ucfirst($e) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div id="items">
                @foreach($compra->detalles as $d)
                    <div class="row g-2 align-items-end item-row mb-2">
                        <div class="col-md-6">
                            ...
                            <select name="asigna_ids[]" class="form-select" required>
                                @foreach($asignaciones as $a)
                                    <option value="{{ $a->id }}" @selected($a->id==$d->asigna_productos_proveedores_id)>
                        ...
                        <div class="col-md-2">
                            ...
                            <input type="number" class="form-control" name="cantidades[]" min="1" value="{{ $d->cantidad }}" required>
                        </div>
                        <div class="col-md-2">
                            <label class="form-label">Costo unit.</label>
                            <input type="number" step="0.01" min="0" class="form-control" name="costos[]" value="{{ $d->costo_unit }}" required>
                        </div>
                        <div class="col-md-2">
                ...
                {{-- fila vacía por si no hay detalles --}}
